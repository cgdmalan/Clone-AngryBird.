...
//  AngryBirdClone
//
//  Created by Çiğdem Alan on 24.05.2023.
//
...
class GameScene: SKScene , SKPhysicsContactDelegate{
    ...
    override func didMove(to view: SKView) {
        ...
        bird = childNode(withName: "birdd") as! SKSpriteNode
        
        let birdtexture = SKTexture(imageNamed: "birdd")
        
        // self.physicsBody = SKPhysicsBody(edgeLoopFrom: frame)
        
        bird.physicsBody = SKPhysicsBody(circleOfRadius: birdtexture.size().width / 32)
                    ...
                    for node in touchNode {
                        ...
                        if let sprite = node as? SKSpriteNode {
                            if sprite == bird {
                                ...
                                let dx = -(touchLocation.x - firstPosition!.x)
                                let dy = -(touchLocation.y - firstPosition!.y)
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if  bird.physicsBody?.velocity.dx ?? 0 <= 0.1 && bird.physicsBody?.velocity.dy ?? 0 <= 0.1 && bird.physicsBody?.angularVelocity ?? 0 <= 0.1 && gameStarted == true {
